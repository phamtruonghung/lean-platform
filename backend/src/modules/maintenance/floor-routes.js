/*
 * The shared floor device's own HTTP surface (issue #77, ADR-0016), mounted
 * by index.js under `/api/maintenance` beside work-order-routes.js.
 *
 * Four routes, and the split between them is the decision:
 *
 *   - POST /floor-devices — an administrator registers one shared device
 *     against one Org Unit and receives its credential exactly once. Reading
 *     the credential back is impossible by construction; only its hash is
 *     stored. Registering real hardware is an operational task out of this
 *     ticket's scope, but a device has to come into existence somehow, and
 *     this is the one door that does it.
 *
 ...
 *
 *   - GET /floor/work-orders — the device presents its credential and reads
 *     the open work at its own Org Unit and beneath it. Nothing else, ever.
 *
 * The write surface itself is NOT duplicated here. A floor write is a POST to
 * the existing `/work-orders/:id/start` and `/complete` routes (issue #63),
 ...
 *
 * `authenticate`/`requireActive`/`findOrgUnit`/`findEmployee` are People's
 * entry point (ADR-0006); the admin gate is a role check on the resolved
 * Account, the same shape `requireAdmin` gives People's own administrator
 * routes. Existence is resolved before anything is written.
 */
...
// Administrator-only, expressed at this Module's own route because People's
// `requireAdmin` is not part of its entry point. Runs after authenticate +
// requireActive, so `req.account` is a real, admitted Account. The wording
// matches the other administrator-only routes in this Module
// (job-plan-routes.js, inventory-routes.js). Only the technician credential is
// gated this way — it is a People-shaped write about one Employee, not an
// Org-Unit-scoped one. Registering a device is scoped by its Org Unit Grant
// (see registerDevice below), not by role.
...
async function registerDevice(req, res, next) {
  try {
    ...
    // Module follows (AGENTS.md §6): findOrgUnit is total, so an unknown id is
...
async function listFloorWorkOrders(req, res, next) {
  try {
    const context = await floorDevices.findDeviceContext(req.floorDevice.id);
