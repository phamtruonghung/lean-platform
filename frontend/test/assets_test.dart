...
void main() {
  ...
  testWidgets('the Asset register is narrowed by a typed term, and typing costs no request', (tester) async {
    ...
    expect(find.byKey(AssetsScreen.rowKey('8')), findsOneWidget);
    expect(find.byKey(AssetsScreen.rowKey('7')), findsOneWidget);
    expect(find.byKey(AssetsScreen.filterCountKey), findsNothing);

    final requestsBefore = wire.requests.length;
    await tester.enterText(find.byKey(AssetsScreen.filterFieldKey), 'conveyor');
    await tester.pumpAndSettle();

    // (a) the rows narrow, (c) the count line says how many of how many.
    expect(find.byKey(AssetsScreen.rowKey('8')), findsOneWidget);
    expect(find.byKey(AssetsScreen.rowKey('7')), findsNothing);
    expect(find.byKey(AssetsScreen.filterCountKey), findsOneWidget);
    ...
    await tester.tap(find.byKey(AssetsScreen.filterClearKey));
    await tester.pumpAndSettle();
